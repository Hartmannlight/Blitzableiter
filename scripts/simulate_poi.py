...
def _build_fake_response(lat: float, lng: float, poi_type: str, poi_id: str, vmax: int | None) -> dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Inject a synthetic POI and run one service cycle.')
    parser.add_argument('--lat', type=float, required=True, help='Latitude inside your GeoJSON area')
    parser.add_argument('--lng', type=float, required=True, help='Longitude inside your GeoJSON area')
    parser.add_argument('--type', dest='poi_type', default='1', help='POI type (e.g., 1 for mobile speed trap, vwd for hazard)')
    ...
    service.run_cycle(now=datetime.now(timezone.utc))
    print('Synthetic POI processed. Check your sender channel for a notification and logs for details.')
